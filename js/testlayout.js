...
$(document).ready(function () {
    ...
    // 비주얼 -----------------------------------------------------------------------------------------------
    let visual_tl = gsap.timeline();
    visual_tl
        .from(".visual .title .big span", { opacity: 0, x: -100, duration: 1})
		.add(() => {
            $(".visual .img-area img").addClass("on");
        }, "-=1")
		.from(".visual .txt-fill", { opacity: 0, x: 50 ,duration:1},'-=50%')
		.add(()=>{
			visual_tl02.play();
		},'-=0.8')
        .from(".visual .title small", { opacity: 0, x: -100, duration: 1 },'-=50%')
		.from('.visual .img-area .txt-box',{x:20,opacity:0,duration:0.5},'-=1')
        .from(".visual .title>span p", { opacity: 0, y: 40 },'-=50%')
		.add(()=>{
			visual_tl03.play();
		},'-=0.5')
    ...
    // 색채워지는 텍스트
    let visual_tl02 = gsap.timeline();
	let visual_tl03 = gsap.timeline();
	...
	visual_tl03
	.fromTo(
		'.visual .img-area .txt-box strong',
        {background: 'linear-gradient(90deg, rgba(0,204,255,1) 0%, rgba(255,255,255,1) 0%)'},
        {background: 'linear-gradient(90deg, rgba(0,204,255,1) 100%, rgba(255,255,255,1) 100%)'}
	)
	.fromTo(
		'.visual .img-area .txt-box span',
        {background: 'linear-gradient(90deg, rgba(0,204,255,1) 0%, rgba(255,255,255,1) 0%)'},
        {background: 'linear-gradient(90deg, rgba(0,204,255,1) 100%, rgba(255,255,255,1) 100%)',duration:2}
	)
	...
	// 인트로 효과
	let intro_svg_box = $('.main-intro .svg-box');
	let intro_svg = $(intro_svg_box).find('svg');
	let intro_path = $(intro_svg).find('path, line, polygon, polyline, circle, ellipse, rect, text, image, use, defs, pattern, mask, clipPath, foreignObject');
	gsap.set($(intro_path),{ drawSVG: '0%'});
	...
	intro_tl
	.to({},{delay:0.2})
	.fromTo('.main-intro .logo-box .wave',1,{y:'100%',opacity:0,},{y:'0',opacity:1,})
	.to('.main-intro .logo-box .wave',1,{left:0})
	.fromTo('.main-intro .logo-box .txt',1,{x:'-30%',opacity:0},{x:'0',opacity:1},'-=1')
	// .to($(intro_path),3,{ drawSVG: '100%'},'-=1');
	.to('.main-intro .bg .gray',3,{x:'100%'},'-=1')
	.to('.main-intro',1,{opacity:0})

	intro_tl.pause();



	 // 인트로 영상을 이미 보았는지 세션스토리지에 저장
	let intro_count = sessionStorage.getItem("intro");
    ...
    $(window).scroll(function () {
        ...
        if (wt >= 10) {
            $(".visual .video-box").addClass("on");
        } else {
            $(".visual .video-box").removeClass("on");
        }
    });
    ...
    ScrollTrigger.create({
        trigger: ".s7 .top .img-box",
        start: "top center",
        onEnter: () => {
            $(".s7 .top .img-box img").addClass("on");
        },
        onLeaveBack: () => {
            $(".s7 .top .img-box img").removeClass("on");
        },
    });

	let s7_tl = gsap.timeline({
		scrollTrigger:{
			trigger:'.s7 .top h3 .top .font-happy',
			start:'top center',
			end:'300',
			toggleActions:'play none none reverse',
		}
	...
	// top 버튼
	$('.s7 .top_btn').on('click', function () {
		TweenMax.to(window, 1, {
			scrollTo: { y: `#scroll_n1`, offsetY: 50 },
